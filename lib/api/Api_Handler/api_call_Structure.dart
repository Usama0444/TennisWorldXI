...
class API_STRUCTURE {
  ...
  Future<Map<String, dynamic>> requestAPI(
      ...
      bool isElasticServerApi = false,
      bool isServerApi = false,
      String rawBody = '',
      bool isEasypaisa = false}) async {
    String api = "";
    ...
    try {
      ...
      debugPrint("request type:->$apiRequestMethod api:->$api");
      ...
      debugPrint("Utils.userToken:-> ${Utils.userToken} \theader:-> $header");
      ...
      switch (e.type) {
        case DioErrorType.connectTimeout:
          CustomToast.showToast(message:"Connection timeout",);
       _exception =  {API_RESPONSE.EXCEPTION: "Connection timeout"};
          break;
        case DioErrorType.sendTimeout:
          CustomToast.showToast(message: "Sent timeout",);
       _exception =  {API_RESPONSE.EXCEPTION: "Sent timeout"};
          break;
        case DioErrorType.receiveTimeout:
          CustomToast.showToast(message:"Receive timeout",);
       _exception =  {API_RESPONSE.EXCEPTION: "Receive timeout"};
          break;
        case DioErrorType.response:
          if(e.response?.statusCode==404){
            CustomToast.showToast(message: "Request not found",);
       _exception =  {API_RESPONSE.EXCEPTION: "Request not found"};
            ...
            CustomToast.showToast(message: "Server error",);
       _exception =  {API_RESPONSE.EXCEPTION: "Server error"};
        ...
        case DioErrorType.cancel:
          CustomToast.showToast(message: "Request cancelled",);
      ...
      if (isShowLoading) progressDialog?.hide();
      CustomToast.showToast(message: "Server Bad response",);
      ...
      CustomToast.showToast(
        message: error.toString().contains("SocketException")
            ? "Internet Connection Error"
            : error.toString(), //API_EXCEPTION.UNKNOWN}",
        );
    ...
    if (response.statusCode >= 200 && response.statusCode < 220) {
      if (response.data["data"]["code"] == 200) {
        if (isWantSuccessMessage) {
          CustomToast.showToast(message: "Request successful",);
        }
      ...
      String error = API_RESPONSE.GetErrorResponse(response.statusCode) ??
